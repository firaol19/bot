    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <Link href={`/dashboard/spot/create?view=${currentMode === 'REAL' ? 'real' : 'demo'}`} className="bg-blue-600 hover:bg-blue-700 text-gray-900 dark:text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition">
                    <Plus size={20} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {bots.map((bot) => (
                    <div key={bot.id} className="bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden relative group">
                        {bot.status === 'RUNNING' && (
                        ...
                        <div className="p-6">
                            <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    <p className="text-gray-600 dark:text-gray-400 text-sm">{bot.symbol}</p>
                                </div>
                            ...
                            <div className="space-y-2 mb-6">
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500 dark:text-gray-500">Capital</span>
                                    <span>${bot.capital}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500 dark:text-gray-500">Profit</span>
                                    <span className={bot.totalProfit >= 0 ? 'text-green-400' : 'text-red-400'}>
                                        ${bot.totalProfit.toFixed(2)}
                                    </span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500 dark:text-gray-500">Open Positions</span>
                ...
                {bots.length === 0 && (
                    <div className="col-span-full border border-dashed border-gray-200 dark:border-gray-800 rounded-xl p-12 text-center text-gray-500 dark:text-gray-500">
